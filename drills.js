...
function findHeight(Bst, counter = 1) {
  if (Bst.left && Bst.right) {
    return Math.max(
      ...[findHeight(Bst.left, counter + 1), findHeight(Bst.right, counter + 1)]
    );
  } else if (Bst.left) {
    ...
    return [
      {
        density:
          thirdLargest(Bst.left).reduce(
            (total, b) => total + Number(b.density),
            0
          ) +
          counter +
          thirdLargest(Bst.right).reduce(
            (total, b) => total + Number(b.density),
            0
          ),
        key: Bst.key,
      ...
      ...thirdLargest(Bst.right, counter),
    ].sort((nodeA, nodeB) => nodeB.density - nodeA.density)[2].key;
  } else if (Bst.left && Bst.right) {
    return [
      {
        density:
...
function isBalanced(bst, distance = 0) {
  ...
  if (bst.left && bst.right) {
    ...
    return [...isBalanced(bst.left, ++distance)];
  } else if (bst.right) {
    return [...isBalanced(bst.right, ++distance)];
  } else {
    return [++distance];
  }
}
